...
angular.module('widget.scrollbar', [])
        ...
        function() {
            return {
                ...
                template: '<div style="position:relative;width:100%;height:100%;">\
                            <div class="ngscroll-content-container" style="display:inline-block;margin-top:0;margin-left:0;width: 100%;" ng-transclude>\
                            </div>\
    ...
    .controller('scrollbarController', [function() {
        ...
        var defaultConfig = {
            dragSpeed: 1, //default browser delta value is 120 or -120
            autoResize: false, // if need auto resize, default false
            autoScroll:true,//是否处理autoScroll事件
            show: false, // if need show when mouse not enter the container element which need scrollbar, default false.
            scrollbar: {
                width: 6, //scrollbar width
                hoverWidth: 8, //scrollbar width when the mouse hover on it
            ...
            compile: function() {
                return function(scope, element, attrs, ctrl) {
                    ...
                    var getScrollbarHeight = function() {
                        var height = Math.max(Math.pow(getContainerHeight(), 2) / getContentHeight() - scrollbarMargin*2,10);
                        return height;
                    };
                    ...
                    var scrollToBottom = function(){
                        var top = Math.min(0, getContainerHeight() - getContentHeight());
                        contentElement.css('margin-top', top + 'px');
                        scrollbar.css('top', -top/getContentHeight()*getContainerHeight() + scrollbarMargin + 'px');
                    }
                    ...
                    var scroll = function(distance) {
                        var minTop = getContainerHeight()-getContentHeight();
                        var newTop = parseInt(contentElement.css('margin-top'), 10) + distance;
                        scrollTo(newTop);
                        if(newTop<minTop&&!isLoading){
                            scope.$emit("pullToLoadingStart");
                            isLoading = true
                        }
                    };

                    scope.$on("pullToLoadingEnd",function(){
                        isLoading = false;
                    })
                    ...
                    $timeout(function() {
                        ...
                        if (!!document.createStyleSheet) { //if the browser is ie browser
                            ...
                            scope.$on("activeModuleChanged",function(event,data){
                                reset();
                            });
                            ...
                            scope.$on("messageDelivered",function(){
                                reset();
                            });

                            scope.$on("resetScrollBar",function(){
                                reset();
                            })
                        }
                    }, 5);
                    ...
                    if(config.autoScroll){
                        scope.$on("autoScroll",function(event,data){
                            scrollTo(getContainerHeight()/2- data.element[0].offsetTop);
                        });
                    }
