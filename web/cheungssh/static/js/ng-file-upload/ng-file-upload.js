...
//        if (attr.ngDisabled) fileElem.attr('disabled', $parse(attr.disabled)(scope));
    ...
    function createFileInput(evt) {
        ...
        if (isInputTypeFile()) {
            ...
            fileElem.css('display', 'none').css('position', 'absolute')
            		.css('z-index', '-100000')
            		.attr('tabindex', '-1');
    ...
    function clickHandler(evt) {
        ...
        if (fileElem) {
        	...
        	function clickAndAssign(evt,openFile) {
        		if (evt != null&&openFile) {
        			fileElem[0].click();
        		}
    	        if (isInputTypeFile()) {
        	...
        	if (navigator.userAgent.toLowerCase().match(/android/)) {
                setTimeout(function() {
                	clickAndAssign(evt,typeof scope.openFile == "undefined"?true:scope.openFile);
                }, 0);        		
        	} else {
        		clickAndAssign(evt,typeof scope.openFile == "undefined"?true:scope.openFile);
        	}
...
function validate(scope, $parse, attr, file, evt) {
    ...
    var fileSizeMax = $parse()(scope, {$file: file, $event: evt}) || 9007199254740991;
